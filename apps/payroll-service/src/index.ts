...
import { componentTypesRouter } from "./routes/componentTypes";
import { taxConfigRouter } from "./routes/taxConfig";
import { salaryStructuresRouter } from "./routes/salaryStructures";
...
app.route("/api/v1/payroll/component-types", componentTypesRouter());
app.route("/api/v1/payroll/tax-config", taxConfigRouter());
app.route("/api/v1/payroll/salary-structures", salaryStructuresRouter());
app.route("/api/v1/payroll/runs", runsRouter());
app.route("/api/v1/payroll/payslips", payslipsRouter());
...
app.onError((err, c) => {
  const logger = createLogger({ service: "payroll-service", requestId: c.get("requestId") });
